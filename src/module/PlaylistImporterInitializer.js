...
export class PlaylistImporterInitializer {
    ...
    Hooks.on('renderPlaylistDirectory', (app, html, data) => {
      ...
      if (game.user?.isGM || game.user?.can('SETTINGS_MODIFY')) {
        ...
        importButton.on('click', (ev) => {
          CONSTANTS.playlistImporter.playlistDirectoryInterface();
        });
      }
    ...
    Hooks.on('renderSettings', (app, html) => {
      ...
      if (game.user?.isGM || game.user?.can('SETTINGS_MODIFY')) {
        ...
        importButton.click((ev) => {
          CONSTANTS.playlistImporter.clearMemoryInterface();
        });
      }
    });
  }
  ...
  static hookReady() {
    Hooks.on('ready', () => {
      CONSTANTS.playlistImporter = new PlaylistImporter();
        ...
        async function (resp) {
          ...
          for (const fileName of localFiles) {
            ...
            if (valid) {
              ...
              debug(`Playlist-Importer: Determined ${fileName} to be of an invalid ext. If you believe this to be an error contact me on Discord.`);
            }
          }
          resolve(true);
        }.bind(this),
